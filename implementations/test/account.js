...
contract('ERC725', function(accounts) {
  ...
  context("Simple tests", async () => {
    ...
    it('should allow the owner to call execute', async function () {
      ...
      await Account.execute(OPERATION_CALL, Counter.address, 0, encodedCall, {from: accounts[0]});
    ...
    it('should not allow non-owner to call execute', async function () {
      ...
      const encodedCall = getEncodedCall(Counter, 'increment');
      await checkErrorRevert(Account.execute(OPERATION_CALL, Counter.address, 0, encodedCall, {from: accounts[1]}), 'Ownable: caller is not the owner');
    ...
    it('should allow owner to send ether', async () => {
      ...
      // Sending 1 ether
      await Account.execute(OPERATION_CALL, Counter.address, oneEth, "0x", {from: accounts[0]});
